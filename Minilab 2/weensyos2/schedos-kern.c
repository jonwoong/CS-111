...
schedule(void)
{
	unsigned int lowestPriority = 0xffffffff;
	...
	else if (scheduling_algorithm == 2) { // p_priority scheduling 
		while (1) {
				// get highest-priority number
				pid_t i;
				for (i = 0; i < NPROCS; i++)
					if (proc_array[i].p_state == P_RUNNABLE &&
						proc_array[i].p_priority < lowestPriority)
						lowestPriority = proc_array[i].p_priority;

				// search first highest-priority task
				pid = (pid + 1) % NPROCS; // to alternate, start with next proc
				if (proc_array[pid].p_state == P_RUNNABLE &&
					proc_array[pid].p_priority <= lowestPriority)
					run(&proc_array[pid]);
			}
	}
